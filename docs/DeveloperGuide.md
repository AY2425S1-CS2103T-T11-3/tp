...
* [**`Model`**](#addressAddressBookModel-component): Holds the data of the App in memory.
...
### Model component
**API** : [`Model.java`](https://github.com/se-edu/addressbook-level3/tree/master/src/main/java/seedu/address/addressAddressBookModel/Model.java)
...
**Note:** An alternative (arguably, a more OOP) addressAddressBookModel is given below. It has a `Tag` list in the `AddressBook`, which `Person` references. This allows `AddressBook` to only require one `Tag` object per unique tag, instead of each `Person` needing their own `Tag` objects.<br>
